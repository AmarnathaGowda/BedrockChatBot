...
import  chatbot_model as model  #**Import your Chatbot file as model

#2 Set Title for Chatbot - https://docs.streamlit.io/library/api-reference/text/st.title
st.title("Hi, This is Chatbot Hope :sunglasses:") # **Modify this based on the title you want in want

#3 LangChain memory to the session cache - Session State - https://docs.streamlit.io/library/api-reference/session-state
if 'memory' not in st.session_state: 
    st.session_state.memory = model.demo_memory() #** Modify the import and memory function() attributes initialize the memory
...
if input_text: 
    ...
    chat_response = model.demo_conversation(input_text=input_text, memory=st.session_state.memory) #** replace with ConversationChain Method name - call the model through the supporting library
